...
const getMerchantOffers = async (req, res) => {
  try {
    ...
    const sanitizedCoupons = allCoupons.map((coupon) => {
      try {
        return {
          ...coupon,
          title: {
          ...
          eligible_for_tier: !!coupon.eligible_for_tier,
        };
      } catch (error) {
        logger.error(`Error sanitizing coupon: ${error.message}`, {
    ...
    let sortedCoupons = sanitizedCoupons;
    if (brandId) {
      sortedCoupons = sanitizedCoupons.sort((a, b) => {
